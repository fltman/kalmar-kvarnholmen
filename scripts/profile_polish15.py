...
R=Path(__file__).resolve().parents[1];start=json.loads((R/'previews/polish15-performance-start.json').read_text())['time'];files=[p for p in (R/'Unreal/Saved/Profiling/CSV').glob('*.csv') if p.stat().st_mtime>start];p=max(files,key=lambda p:p.stat().st_mtime)
...
report={'status':'measured_editor_only' if frames else 'invalid_no_rendered_frames','source_csv':str(p.relative_to(R)),'context':'Stationary Fiskaregatan editor viewport; current viewport resolution and scalability, not packaged gameplay or controlled before/after comparison.','warmup_seconds':8,'rendered_frames':rendered,'frame_time':q(frames),'gpu_time':q(gpu),'approx_median_fps':1000/statistics.median(frames) if frames else None};(R/'previews/polish15-performance.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
