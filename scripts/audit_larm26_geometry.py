...
names=json.loads((R/'previews/larm26-build.json').read_text())['changed'];result={}
for name in names:
...
p=R/'previews/larm26-repeatability.json';previous=json.loads(p.read_text()) if p.exists() else None
...
p.write_text(json.dumps(report,indent=2));print('LARM26_GEOMETRY_OK',bool(previous))
