...
extension_names=['SM_Kvarnholmen_House_93199604'];extension_cameras=district17_cameras
for bid in []:
...
tail=(R/'scripts/rebuild_polish15.py').read_text();tail=tail[tail.index("exec(compile((R/'scripts/district_tangent_export.py')"):];tail=tail.replace('polish15-build.json','district17-klapp-build.json').replace('POLISH15_BUILD_OK','DISTRICT17_ROOF_OK').replace('EXPORT15','EXPORT17_ROOF')
...
for bid in []:a['SM_Kvarnholmen_House_'+bid]['roof']='sealed flat roof following mapped footprint and courtyard rings'
