...
tree=ast.parse((R/'scripts/build_storgatan.py').read_text());tree.body=[n for n in tree.body if isinstance(n,ast.FunctionDef)]
exec(compile(tree,'street_helpers','exec'))
exec(compile((R/'scripts/build_larmtorget_facades.py').read_text(),str(R/'scripts/build_larmtorget_facades.py'),'exec'))
extension_names=larm_names;extension_cameras=larm_cameras
audit={}
for name in extension_names:
 obj=bpy.data.objects[name];me=obj.data;me.calc_loop_triangles();uv=me.uv_layers.active.data
 ...
 audit[name]={'vertices':len(me.vertices),'polygons':len(me.polygons),'reprojected_polygons':len(repaired),'zero_uv_triangles':zeros}
 bpy.ops.object.select_all(action='DESELECT');obj.select_set(True);bpy.context.view_layer.objects.active=obj
 bpy.ops.export_scene.fbx(filepath=str(R/'exports/meshes'/(name+'.fbx')),use_selection=True,object_types={'MESH'},axis_forward='-Y',axis_up='Z',apply_unit_scale=True,global_scale=1,bake_space_transform=False,mesh_smooth_type='FACE',use_tspace=True,add_leaf_bones=False,path_mode='AUTO')
...
(R/'previews/larmtorget-facades-build.json').write_text(json.dumps({'status':'passed' if not any(x['zero_uv_triangles'] for x in audit.values()) else 'uv_review_required','assets':audit,'changed':extension_names,'cameras':[c[0] for c in extension_cameras]},indent=2));print('LARMTORGET_BUILD_OK')
