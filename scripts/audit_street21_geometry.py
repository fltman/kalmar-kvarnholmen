...
names=json.loads((R/'previews/street21-build.json').read_text())['changed'];result={}
for name in names:
...
p=R/'previews/street21-repeatability.json';previous=json.loads(p.read_text()) if p.exists() else None
...
p.write_text(json.dumps(report,indent=2));print('STREET21_GEOMETRY_OK',bool(previous))
