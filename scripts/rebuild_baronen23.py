...
exec(compile((R/'scripts/build_baronen23.py').read_text(),'build_baronen23.py','exec'))
extension_names=baronen23_names;extension_cameras=baronen23_cameras
tail=(R/'scripts/rebuild_polish15.py').read_text();tail=tail[tail.index("exec(compile((R/'scripts/district_tangent_export.py')"):];tail=tail.replace('polish15-build.json','baronen23-build.json').replace('POLISH15_BUILD_OK','BARONEN23_BUILD_OK')
exec(compile(tail,'export_baronen23','exec'))
