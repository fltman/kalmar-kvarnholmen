...
exec(compile((R/'scripts/build_castle27.py').read_text(),'build_castle27.py','exec'))
extension_names=castle27_names;extension_cameras=castle27_cameras
tail=(R/'scripts/rebuild_polish15.py').read_text();tail=tail[tail.index("exec(compile((R/'scripts/district_tangent_export.py')"):];tail=tail.replace('polish15-build.json','castle27-build.json').replace('POLISH15_BUILD_OK','CASTLE27_BUILD_OK')
exec(compile(tail,'export_castle27','exec'))
