...
exec(compile((R/'scripts/build_station24.py').read_text(),'build_station24.py','exec'))
extension_names=station24_names;extension_cameras=station24_cameras
tail=(R/'scripts/rebuild_polish15.py').read_text();tail=tail[tail.index("exec(compile((R/'scripts/district_tangent_export.py')"):];tail=tail.replace('polish15-build.json','station24-build.json').replace('POLISH15_BUILD_OK','STATION24_BUILD_OK')
exec(compile(tail,'export_station24','exec'))
