...
if (R/'source/kvarnholmen.json').exists():
 ...
 extension_names=list(dict.fromkeys(extension_names+district_names));extension_cameras+=district_cameras
audit={}
for name in extension_names:
 ...
 if obj.name.startswith('SM_Kvarnholmen_') or obj.name in globals().get('sodra_names',[])+globals().get('street16_names',[])+globals().get('district17_names',[]):
  ...
  bpy.ops.export_scene.fbx(filepath=str(R/'exports/meshes'/(name+'.fbx')),use_selection=True,object_types={'MESH'},axis_forward='-Y',axis_up='Z',apply_unit_scale=True,global_scale=1,bake_space_transform=False,mesh_smooth_type='FACE',use_tspace=obj.name in globals().get('larm_names',[]),add_leaf_bones=False,path_mode='AUTO')
...
(R/'previews/storgatan-build.json').write_text(json.dumps({'status':'passed' if not any(x['zero_uv_triangles'] for x in audit.values()) else 'uv_review_required','assets':audit,'changed':extension_names,'cameras':[c[0] for c in extension_cameras]},indent=2));print('STORGATAN_BUILD_OK')
