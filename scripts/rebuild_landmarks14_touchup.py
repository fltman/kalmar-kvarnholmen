...
extension_names=['SM_Kvarnholmen_House_92204165','SM_Kvarnholmen_House_91846928','SM_Kvarnholmen_Vall_Regeringen'];extension_cameras=landmark_cameras
...
(R/'previews/landmarks14-touchup-build.json').write_text(json.dumps({'status':'passed' if not any(x['zero_uv_triangles'] for x in audit.values()) else 'uv_review_required','assets':audit,'changed':extension_names,'cameras':[c[0] for c in extension_cameras]},indent=2));print('LANDMARKS14_BUILD_OK')
