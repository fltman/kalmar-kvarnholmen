...
names=json.loads((R/'previews/castle27-build.json').read_text())['changed'];result={}
for name in names:
...
p=R/'previews/castle27-repeatability.json';previous=json.loads(p.read_text()) if p.exists() else None
...
p.write_text(json.dumps(report,indent=2));print('CASTLE27_GEOMETRY_OK',bool(previous))
