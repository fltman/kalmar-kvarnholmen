...
report={'status':'importing','meshes':[],'materials':{},'checks':{},'facade_provenance':'267 inferred facades; landmark reference notes preserved'}
ionic_only=False
try:
 for name in [n for n in MAN['materials'] if n in json.loads((R/'source/polish15-material-specs.json').read_text())]:
 ...
 prefixes=tuple(json.loads((R/'previews/polish15-build.json').read_text())['changed'])
 ...
 for spec in selected:
  ...
  if actor:
   actor.static_mesh_component.set_static_mesh(mesh);actor.set_folder_path(spec['category'])
  else:
  ...
  (R/'previews/polish15-import.json').write_text(json.dumps(report,indent=2))
 for spec in MAN['cameras']:
  if not spec['name'].startswith(tuple(str(i)+'_' for i in range(59,69))):continue
 ...
 report['status']='failed';report['error']=traceback.format_exc();(R/'previews/polish15-import.json').write_text(json.dumps(report,indent=2));raise


exec(compile((R/'Unreal/Content/Python/review_hero.py').read_text(),str(R/'Unreal/Content/Python/review_hero.py'),'exec'),globals())
(R/'previews/hero-review-request.json').write_text(json.dumps({'shots':[['59_TrippTrappTrull','88_Polish_Trio.png'],['65_Vasterport','89_Polish_Vasterport.png'],['49_Fiskaregatan','90_Polish_Fiskaregatan.png'],['48_Norra_Langgatan','91_Polish_Norra.png'],['62_Angkvarnen','92_Polish_Angkvarnen.png'],['68_Kavaljersporten','93_Polish_Kavaljersporten.png']]}))
