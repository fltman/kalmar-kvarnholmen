...
for road in data['checks']:
 ...
 for p,q in zip(road['points'],road['points'][1:]):
  ...
  if hit is not None:
   ...
   try:
    component=hit.get_editor_property('component');actor=component.get_owner() if component else None;location=hit.get_editor_property('impact_point')
...
(R/'previews/sodra-facades-collision.json').write_text(json.dumps(report,indent=2,ensure_ascii=False))
