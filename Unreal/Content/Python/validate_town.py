...
selected=set(json.loads((R/'previews/town-pass8-build.json').read_text())['updated'])
...
report={'status':'passed' if all(checks.values()) else 'failed','time':datetime.datetime.now().isoformat(),'scope':'Visibility collision rays at pedestrian height against the twelve modified buildings; furniture excluded','checks':checks}
(R/'previews/town-pass8-walkways.json').write_text(json.dumps(report,indent=2));u.get_editor_subsystem(u.LevelEditorSubsystem).save_current_level()
