"""Actual PIE traversal: Södra Långgatan, then Östra Sjögatan to Fiskaregatan."""
import unreal as u,time,json,traceback,math
from pathlib import Path
R=Path(__file__).resolve().parents[3];route=json.loads((R/'source/sodra-facades-walk.json').read_text())
...
def write():(R/'previews/sodra-facades-pie-walk.json').write_text(json.dumps(report,indent=2))
def finish():
