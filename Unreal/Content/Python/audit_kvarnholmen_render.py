...
names=json.loads((R/'previews/kvarnholmen-build.json').read_text())['changed']
for name in names:
 if not name.startswith('SM_Kvarnholmen_'):continue
...
(R/'previews/kvarnholmen-render-audit.json').write_text(json.dumps(report,indent=2))
for actor in u.get_editor_subsystem(u.EditorActorSubsystem).get_all_level_actors():
 if actor.get_actor_label()=='Kvarnholmen_Water':actor.static_mesh_component.set_collision_enabled(u.CollisionEnabled.NO_COLLISION)
