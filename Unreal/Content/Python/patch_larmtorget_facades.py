...
ionic_only=True
try:
 for name in ['M_Town_Ivory','M_Town_Yellow','M_Town_PaintWhite','M_Town_PaintGreen','M_Town_PaintBrown','M_Town_Copper']:
 ...
 prefixes=tuple(json.loads((R/'previews/larmtorget-facades-build.json').read_text())['changed'])
 ...
 for spec in selected:
  ...
  changes={'recompute_normals':False,'recompute_tangents':name in ('SM_Storgatan_Frimurarehotellet','SM_Building_91846938','SM_Building_91264999'),'remove_degenerates':True,'use_full_precision_u_vs':True,'max_lumen_mesh_cards':32,'generate_distance_field_as_if_two_sided':True}
  ...
  if not nanite.enabled:nanite.enabled=True;smes.set_nanite_settings(mesh,nanite,True)
  if name in ('SM_Storgatan_Frimurarehotellet','SM_Building_91846938','SM_Building_91264999'):
   n=smes.get_nanite_settings(mesh)
   if not n.explicit_tangents or n.fallback_relative_error!=0.0 or n.fallback_target!=type(n.fallback_target).RELATIVE_ERROR:
    n.explicit_tangents=True;n.generate_fallback=type(n.generate_fallback).ENABLED;n.fallback_target=type(n.fallback_target).RELATIVE_ERROR;n.fallback_relative_error=0.0
    smes.set_nanite_settings(mesh,n,True)
  ...
  (R/'previews/larmtorget-facades-import.json').write_text(json.dumps(report,indent=2))
 for spec in MAN['cameras']:
  if not spec['name'].startswith(('41_','42_','43_','44_','45_')):continue
 ...
 report['checks']['mesh_integrity']=bool(report['meshes']) and all(m['bounds_finite'] and m['nanite'] and m['materials_assigned'] for m in report['meshes'])
 ...
 report['status']='failed';report['error']=traceback.format_exc();(R/'previews/larmtorget-facades-import.json').write_text(json.dumps(report,indent=2));raise

exec(compile((R/'Unreal/Content/Python/review_hero.py').read_text(),str(R/'Unreal/Content/Python/review_hero.py'),'exec'),globals())
(R/'previews/hero-review-request.json').write_text(json.dumps({'shots':[['41_Larmtorget_Norra','62_Larmtorget_North.png'],['42_Larmtorget_Ludvig','63_Larmtorget_Ludvig.png'],['43_Larmtorget_Frimurare_Detalj','64_Larmtorget_Frimurare.png'],['44_Larmtorget_Teater_Fasad','65_Larmtorget_Theatre.png']]}))
