...
R=Path(__file__).resolve().parents[3];route=json.loads((R/'source/kvarnholmen-walk.json').read_text())
...
def write():(R/'previews/kvarnholmen-pie-walk.json').write_text(json.dumps(report,indent=2))
def finish():
