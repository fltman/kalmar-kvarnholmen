...
ionic_only=False
try:
 for name in ['M_Storgatan_Setts','M_Storgatan_Slabs']:
 ...
 prefixes=tuple(json.loads((R/'previews/storgatan-build.json').read_text())['changed'])
 ...
 for spec in selected:
  ...
  data=opt.static_mesh_import_data;data.build_nanite=not (name.startswith('SM_Kvarnholmen_') or name=='SM_Building_91846951');data.combine_meshes=True;data.auto_generate_collision=False;data.generate_lightmap_u_vs=False;data.convert_scene=True;data.convert_scene_unit=True;data.force_front_x_axis=False;data.normal_import_method=u.FBXNormalImportMethod.FBXNIM_IMPORT_NORMALS_AND_TANGENTS if name.startswith('SM_Kvarnholmen_') or name=='SM_Building_91846951' else u.FBXNormalImportMethod.FBXNIM_IMPORT_NORMALS
  ...
  changes={'remove_degenerates':True,'use_full_precision_u_vs':True,'max_lumen_mesh_cards':32,'generate_distance_field_as_if_two_sided':True}
  if name.startswith('SM_Kvarnholmen_') or name=='SM_Building_91846951':changes['recompute_tangents']=False
  ...
  if nanite.enabled!= (not (name.startswith('SM_Kvarnholmen_') or name=='SM_Building_91846951')):nanite.enabled=not (name.startswith('SM_Kvarnholmen_') or name=='SM_Building_91846951');smes.set_nanite_settings(mesh,nanite,True)
  ...
  report['meshes'].append({'name':name,'vertices':spec['vertices'],'polygons':spec['polygons'],'bounds_finite':finite,'nanite':bool(smes.get_nanite_settings(mesh).enabled),'materials_assigned':all(mesh.get_material(i) is not None for i in range(len(mesh.static_materials)))})
  (R/'previews/storgatan-import.json').write_text(json.dumps(report,indent=2))
 for spec in MAN['cameras']:
  if not spec['name'].startswith(('34_','35_','36_','37_','38_','39_','40_','46_','47_','48_','49_','50_','51_','52_','53_','54_','55_','56_','57_','58_','59_','60_','61_','62_','63_','64_','65_','66_','67_','68_')):continue
 ...
 report['checks']['mesh_integrity']=bool(report['meshes']) and all(m['bounds_finite'] and m['nanite']==(not (m['name'].startswith('SM_Kvarnholmen_') or m['name']=='SM_Building_91846951')) and m['materials_assigned'] for m in report['meshes'])
 ...
 report['status']='failed';report['error']=traceback.format_exc();(R/'previews/storgatan-import.json').write_text(json.dumps(report,indent=2));raise

exec(compile((R/'Unreal/Content/Python/review_hero.py').read_text(),str(R/'Unreal/Content/Python/review_hero.py'),'exec'),globals())
(R/'previews/hero-review-request.json').write_text(json.dumps({'shots':[['34_Storgatan_Entré','56_Storgatan_Entry.png'],['36_Storgatan_Kaggensgatan','57_Storgatan_Middle.png'],['37_Larmtorget_Storgatan','58_Larmtorget_East.png'],['38_Larmtorget_Teatern','59_Larmtorget_Theatre.png'],['39_Larmtorget_Frimurare','60_Larmtorget_Hotel.png'],['40_Storgatan_Översikt','61_Storgatan_Overview.png']]}))
