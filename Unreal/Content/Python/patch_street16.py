"""Replace only the nine authored street frontages; preserve world and materials."""
...
tree=ast.parse((R/'Unreal/Content/Python/build_stortorget.py').read_text());tree.body=[n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name in {'import_task','vec','look'}];exec(compile(tree,'street16_import_helpers','exec'))
names=json.loads((R/'previews/street16-build.json').read_text())['changed']
names=globals().get('street16_import_only',names)
report_file=globals().get('street16_import_report','street16-import.json')
...
try:
 ...
 for spec in MAN['assets']:
  ...
  for k,v in {'recompute_normals':False,'recompute_tangents':False,'remove_degenerates':True,'use_full_precision_u_vs':True,'max_lumen_mesh_cards':32,'generate_distance_field_as_if_two_sided':True}.items():build.set_editor_property(k,v)
  smes.set_lod_build_settings(mesh,0,build)
  n=smes.get_nanite_settings(mesh);n.enabled=True;n.explicit_tangents=True;n.generate_fallback=type(n.generate_fallback).ENABLED;n.fallback_target=type(n.fallback_target).RELATIVE_ERROR;n.fallback_relative_error=0
  smes.set_nanite_settings(mesh,n,True);library.save_loaded_asset(mesh)
 ...
 for s in MAN['cameras']:
  if not s['name'].startswith(('69_','70_')):continue
