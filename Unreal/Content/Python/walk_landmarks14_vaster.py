"""Actual PIE traversal through Jordbroporten, along Ölandsgatan, through Kavaljersporten."""
import unreal as u,time,json,traceback,math
from pathlib import Path
R=Path(__file__).resolve().parents[3];route=json.loads((R/'source/landmarks14-vaster-walk.json').read_text())
...
def write():(R/'previews/landmarks14-vaster-pie-walk.json').write_text(json.dumps(report,indent=2))
def finish():
