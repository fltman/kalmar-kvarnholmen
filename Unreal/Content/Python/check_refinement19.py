...
names=json.loads((R/'previews/refinement19-build.json').read_text())['changed']
for name in names:
...
(R/'previews/refinement19-render-audit.json').write_text(json.dumps(report,indent=2))

P=Path(__file__).resolve().parent
code=(P/'validate_storgatan.py').read_text().replace('storgatan-collision.json','refinement19-storgatan-collision.json')
exec(compile(code,str(P/'validate_storgatan.py'),'exec'),{'__file__':str(P/'validate_storgatan.py')})
