"""Selective pass-6 import; preserve the hand-reviewed lighting and other assets."""
...
ionic_only=False
try:
 for name in ['M_Church_Cut_Stone','M_Church_Window_Frame','M_Church_Window_Glass','M_Church_Lantern_Shutter','M_Hymn_Board']:
 ...
 prefixes=('SM_Kalmar_Domkyrka','SM_Domkyrka_Exterior_Details','SM_Domkyrka_Altar_Pulpit','SM_Domkyrka_Altar_Carving','SM_Domkyrka_Chandeliers','SM_Domkyrka_Hymn_Boards','SM_Domkyrka_Pulpit_Support')
 ...
 for spec in selected:
  ...
  if actor:actor.static_mesh_component.set_static_mesh(mesh)
   ...
   actor=actors.spawn_actor_from_object(mesh,u.Vector(),u.Rotator());actor.set_actor_label(label);actor.set_folder_path('Stortorget/'+spec['category']);actor.static_mesh_component.set_mobility(u.ComponentMobility.STATIC)
  ...
  (R/'previews/exterior-pass6-import.json').write_text(json.dumps(report,indent=2))
 for spec in MAN['cameras']:
  if not spec['name'].startswith(('16_','17_','18_','19_')):continue
 ...
 exec(compile((R/'Unreal/Content/Python/review_hero.py').read_text(),str(R/'Unreal/Content/Python/review_hero.py'),'exec'))
 (R/'previews/hero-review-request.json').write_text(json.dumps({'shots':[['16_Predikstol_Infästning','36_Unreal_Pulpit_Mounted.png'],['17_Psalmtavla_Infästning','37_Unreal_Hymn_Boards_Mounted.png'],['18_Exteriör_Fasad','38_Unreal_Exterior_Pass6.png'],['19_Tornhuv','39_Unreal_Tower_Pass6.png']]}))
except Exception:
 report['status']='failed';report['error']=traceback.format_exc();(R/'previews/exterior-pass6-import.json').write_text(json.dumps(report,indent=2));raise
