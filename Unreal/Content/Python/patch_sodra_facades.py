...
ionic_only=False
try:
 for name in ['M_Sodra_'+q for q in ['Cream','Sand','RedWood','OchreWood','DarkTile']]:
 ...
 prefixes=tuple(json.loads((R/'previews/sodra-facades-build.json').read_text())['changed'])
 ...
 for spec in selected:
  ...
  (R/'previews/sodra-facades-import.json').write_text(json.dumps(report,indent=2))
 for spec in MAN['cameras']:
  if not spec['name'].startswith(('54_','55_','56_','57_','58_')):continue
 ...
 report['status']='failed';report['error']=traceback.format_exc();(R/'previews/sodra-facades-import.json').write_text(json.dumps(report,indent=2));raise

exec(compile((R/'Unreal/Content/Python/review_hero.py').read_text(),str(R/'Unreal/Content/Python/review_hero.py'),'exec'),globals())
(R/'previews/hero-review-request.json').write_text(json.dumps({'execute':'check_sodra_facades.py','shots':[['54_Sodra_Hotellhorn','73_Sodra_Hotel_Corner.png'],['55_Sodra_Fasadstrak','74_Sodra_Frontages.png'],['56_Sodra_Trahus','75_Sodra_Timber_Houses.png'],['57_Sodra_NorraHornet','76_Sodra_North_Corner.png'],['58_Sodra_Oversikt','77_Sodra_Overview.png']]}))
