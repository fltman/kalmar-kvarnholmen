...
try:
 ...
 for spec in selected:
  ...
  (R/'previews/landmarks14-import.json').write_text(json.dumps(report,indent=2))
 ...
 report['status']='failed';report['error']=traceback.format_exc();(R/'previews/landmarks14-import.json').write_text(json.dumps(report,indent=2));raise

exec(compile((R/'Unreal/Content/Python/review_hero.py').read_text(),str(R/'Unreal/Content/Python/review_hero.py'),'exec'),globals())

(R/'previews/hero-review-request.json').write_text(json.dumps({'execute':'check_landmarks14.py','shots':[[c['name'],str(78+i)+'_'+c['name'][3:]+'.png'] for i,c in enumerate([c for c in MAN['cameras'] if 59<=int(c['name'][:2])<=68])]}))
