...
tree=ast.parse((R/'Unreal/Content/Python/build_stortorget.py').read_text());tree.body=[n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name in {'import_task','vec','look'}];exec(compile(tree,'pass18_import_helpers','exec'))
names=json.loads((R/'previews/pass18-build.json').read_text())['changed']
names=globals().get('pass18_import_only',names)
report_file=globals().get('pass18_import_report','pass18-import.json')
...
try:
 ...
 for spec in MAN['assets']:
  ...
  if not actor:
   actor=actors.spawn_actor_from_class(u.StaticMeshActor,u.Vector(0,0,0));actor.set_actor_label(name.removeprefix('SM_'));actor.set_folder_path(spec['category'])
 ...
 for s in MAN['cameras']:
  if not s['name'].startswith(tuple(str(i)+'_' for i in range(78,91))):continue
