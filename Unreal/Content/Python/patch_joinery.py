"""Selective pass-5 import; preserve the hand-reviewed lighting and other assets."""
...
ionic_only='-IonicOnly' in u.SystemLibrary.get_command_line()
try:
 for name in ['M_Pew_Grey','M_Pew_Inset','M_Pew_Trim','M_Pew_Ornament']:
 ...
 prefixes=('SM_Domkyrka_Ionic_Details',) if ionic_only else ('SM_Domkyrka_Pews','SM_Domkyrka_Ionic_Details')
 ...
 for spec in selected:
  ...
  (R/'previews/joinery-pass5-import.json').write_text(json.dumps(report,indent=2))
 for spec in MAN['cameras']:
  if not spec['name'].startswith(('14_','15_')):continue
 ...
 (R/'previews/joinery-pass5-import.json').write_text(json.dumps(report,indent=2))
 if report['status']!='passed':raise RuntimeError('Joinery verification failed')
 ...
 report['status']='failed';report['error']=traceback.format_exc();(R/'previews/joinery-pass5-import.json').write_text(json.dumps(report,indent=2));raise
